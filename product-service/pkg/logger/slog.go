...
import (
	...
	"gitlab.crja72.ru/golang/2025/spring/course/projects/go7/marketplace/product-microservice/pkg/kafka"
)
...
func New(env string, kafkaBrokers []string) *Logger {
		...
		log = slog.New(
			slogformatter.NewFormatterHandler(
				...
				Next: slog.NewTextHandler(os.Stdout, &slog.HandlerOptions{
					...
					AddSource: false, // Отключаем автоматическое добавление source
				}),
				NoBaggage:     false,
				NoTraceEvents: false,
			}))
	...
	if kafkaBrokers != nil || len(kafkaBrokers) > 0 {
		...
		if err != nil {
			log.Warn("Something trouble with create new async producer", Err(err))
		}
...
func (l *Logger) logWithSource(level slog.Level, msg string, args ...any) {
	...
	_, file, line, ok := runtime.Caller(2) // 2 уровня вверх: вызов logWithSource -> Info/Error/...
	if !ok {
		file = "unknown"
		line = 0
	}

	// Добавляем source в аргументы
